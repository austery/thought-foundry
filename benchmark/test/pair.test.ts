...
import { mkdtemp, mkdir, writeFile, readFile, cp, symlink, readdir } from 'node:fs/promises';
...
for(const native of [false,true]) test(`real ${native?'native':'hybrid'} added-article pair primes independent caches and indexes the fixture in both engines`,async()=>{
  ...
  if(native) for(const name of await readdir(join(baseline,'src'))) if(name.endsWith('.njk')) await cp(join(baseline,'src',name),join(site,'src',name));
  await writeFile(join(site,'src/content/notes/Case.md'),'---\ntitle: Small corpus\ndate: "2026-01-01"\nlayout: post.njk\ntags: [note]\n---\n# Example\n\n中文内容测试。');
  const reports=join(root,'reports');
  await runPair('added',1,site,join(root,'builds'),reports,native);
