...
- [x] Use Toronto publication dates consistently, including midnight/DST cases.
  Missing-date material remains reachable and is not assigned an invented date.
- [x] Preserve existing source/day reading units and all public feed URLs; keep
  historical pagination reachable while introducing date-first discovery.
- [x] Offer only dates backed by saved material; do not generate empty day pages.
- [x] Reuse the existing long definition: more than 280 Unicode code points.
  Hide zero long counts; show an honest empty state when filters match nothing.
- [x] Do not create per-post pages, a new long-content route, or new content fields.
- [x] Test exclusion, duplicate identities, multiple sources/days, unknown dates,
